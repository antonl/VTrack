classdef Container < hgsetget
    ...
    methods
        ...
        function set.Enable( obj, value )
            ...
            ch = obj.Children_;
            for ii=1:numel( ch )
                helpSetChildEnable( ch(ii), obj.Enable_ );
            end
        ...
        function repositionChild( obj, child, position )
            ...
            if position(3)<=0 || position(4)<=0
                ...
                elseif strcmpi( oldunits, 'Normalized' )
                    ...
                    % normalised position [0,0] we need to subtract one
                    % from the position part.
                    set( child, propname, (position-[1 1 0 0])./[w h w h] );
                else
    ...
    methods( Access = private, Sealed = true )
        ...
        function onChildAddedEvent( obj, source, eventData ) %#ok<INUSL>
            ...
            if strcmpi( obj.Enable_, 'off' )
                helpSetChildEnable( child, obj.Enable_ );
            end
